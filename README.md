#  Hey, moi c’est Sugdenaz

 Étudiante en **BUT Informatique**  
 Je transforme des idées en code (et parfois en bugs)  
 Curieuse, têtue, et étonnamment patiente avec les messages d’erreur

---

##  À propos de moi

-  Je bosse sur des projets **web & full-stack**
-  J’apprends en cassant des trucs (puis en les réparant)
-  J’aime tester, comprendre, optimiser
-  Peut contenir du code qui marche *par accident*

---

##  Tech stack (aka mon terrain de jeu)

### Langages
![JavaScript](https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-000?style=for-the-badge&logo=typescript)
![PHP](https://img.shields.io/badge/PHP-000?style=for-the-badge&logo=php)
![Python](https://img.shields.io/badge/Python-000?style=for-the-badge&logo=python)
![SQL](https://img.shields.io/badge/SQL-000?style=for-the-badge&logo=postgresql)

### Frontend
![React](https://img.shields.io/badge/React-000?style=for-the-badge&logo=react)
![Vue.js](https://img.shields.io/badge/Vue.js-000?style=for-the-badge&logo=vuedotjs)
![Tailwind](https://img.shields.io/badge/Tailwind-000?style=for-the-badge&logo=tailwindcss)
![Vuetify](https://img.shields.io/badge/Vuetify-000?style=for-the-badge&logo=vuetify)

### Backend & DB
![Node.js](https://img.shields.io/badge/Node.js-000?style=for-the-badge&logo=nodedotjs)
![Express](https://img.shields.io/badge/Express-000?style=for-the-badge&logo=express)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=for-the-badge&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-000?style=for-the-badge&logo=mongodb)
![Prisma](https://img.shields.io/badge/Prisma-000?style=for-the-badge&logo=prisma)

### Autres trucs utiles
![Git](https://img.shields.io/badge/Git-000?style=for-the-badge&logo=git)
![Electron](https://img.shields.io/badge/Electron-000?style=for-the-badge&logo=electron)
![Docker](https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker)

---

##  Projets

 Projets universitaires (mais faits avec amour)  
 Applications **React / Vue / Electron**  
 Projets full-stack avec API maison  
 Un peu d’IA, quand ça veut bien coopérer

---

##  Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=TON_USERNAME&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=TON_USERNAME&layout=compact&theme=tokyonight)

---

##  Actuellement
-  Montée en compétence **React + TypeScript**
-  Construction de projets solides (et présentables)

---

##  Me retrouver
-  GitHub : ici même, tu es déjà trop loin pour reculer
-  LinkedIn : si tu veux faire ça sérieusement

---

⭐ Si tu trouves un repo utile, cool  
⭐ Si tu le forks, encore mieux  
⭐ Si ça bug… c’est sûrement voulu
